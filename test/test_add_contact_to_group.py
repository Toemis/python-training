...
def test_add_one_contact_to_group(app, db, orm):
    # if there are no contact/groups OR no groups without contacts create them
    if len(db.get_contact_list()) == 0 or not get_all_not_full_groups_and_contacts(db, orm):
        app.contact.create(Contact(first_name="New", last_name="New"))
    if len(db.get_group_list()) == 0 or not get_all_not_full_groups_and_contacts(db, orm):
        app.group.create(Group(name="test"))
    # select random group and contact not belong to it to add contact to the group
    not_paired_groups_and_contacts = get_all_not_full_groups_and_contacts(db, orm)
...
def get_all_not_full_groups_and_contacts(db, orm):
    all_groups = db.get_group_list()
    # create a dict with group as a key and contact that not in this group as a values
    empty_pairs = {}
    for gr in all_groups:
        if orm.get_contacts_not_in_group(gr):  # if this list not empty
            empty_pairs[gr.id] = orm.get_contacts_not_in_group(gr)  # add this group and contacts into the dict
    return empty_pairs
